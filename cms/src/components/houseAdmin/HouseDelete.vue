      ...
      <div class="container" v-if="isDelete">
        ...
        <div class="content">
          ...
          <div class="messageval">
            <textarea v-model="deleteObj.desc" placeholder="输入原因(选填)"></textarea>
          </div>
        </div>
    ...
    methods: {
      button_click () {
        ...
        if (this.operType === 'delete') {
          url = _this.$store.state.api.resources_adjust.get_houseLine_delete
        } else if (this.operType === 'enable') {
          url = _this.$store.state.api.resources_adjust.get_houseLine_use
          this.deleteObj.status = '0';
        } else if (this.operType === 'disable') {
          url = _this.$store.state.api.resources_adjust.get_houseLine_use
          this.deleteObj.status = '1';
        } else if (this.operType === 'yuangong-delete') {
          url = _this.$store.state.api.resources_adjust.get_houseLine_delete
        } else {
