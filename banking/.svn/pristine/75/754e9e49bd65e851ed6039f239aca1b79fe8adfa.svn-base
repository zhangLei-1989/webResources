<template>
      ...
      <el-col :span="24">
        <el-form :inline="true" class="demo-form-inline" style="display:flex;;">
          <el-form-item label="金额">
            <el-input placeholder="最低" style="width:100px;" v-model="screen.minPrice"></el-input>
            —
            <el-input placeholder="最高" style="width:100px;" v-model="screen.maxPrice"></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-table :data="table.tbody" stripe @row-click="tableRowClick">
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.status==0" class="finance-state orange">待打款</span>
          ...
          <el-table-column
            v-if="status_index==1 || status_index==2"
            label="审核人/审核时间/审核备注">
            <template slot-scope="scope">
                <p>
                  <span style="margin-right:7px;">{{ scope.row.checkName }}</span>
                  <span>{{ scope.row.checkDate }}</span>
                </p>
                <p>{{ '备注：' + scope.row.breakcontractReason }}</p>
            </template>
          </el-table-column>
          <el-table-column
            prop="handle"
            label="操作"
            :width="status_index==0?120:140">
            <template slot-scope="scope">
              <el-button type="primary" v-if="status_index==0" plain @click.stop="openDetail2(scope.row.id)">审核</el-button>
              <el-button type="primary" v-if="status_index!=0" plain @click="dialogType='detail'" style="padding-left:0px;padding-right:0px;">查看合同</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog
        ...
        <el-col :span="24">
          ...
          <el-row>
            <el-col :span="24" style="position: relative;">
              <div style="position:absolute;right:30px;line-height:40px;font-size:14px;color:#188ff0;cursor:pointer;z-index:1;" @click="seeZip()">打包下载所有附件</div>
      ...
      <template v-if="dialog.detail_msg.state==3 && dialogType!='detail'" slot="footer">
        <el-button type="danger" v-show="isButtonAuthor('433c4e40a0fa48afb968e8d0a7ab9a95')" style="background-color:#f2082e !important;border-color:#f2082e !important;" @click="openReject(dialog.detail_msg.id)">驳回</el-button>
        <el-button type="primary" v-show="isButtonAuthor('3c64fa5789894d649fe455e542a57552')" style="width:auto;border-radius:4px;" @click="openAdopt(dialog.detail_msg.id)">通过</el-button>
      </template>
    </el-dialog>
    <el-dialog
      title="驳回合同"
  ...
  methods: {
    ...
    async openDetail(id) {
      ...
      if (detailData.status.code == 200) {
        ...
        self.dialog.detail_msg.id = contract.id;
        ...
        if (contract.status == 0) {
          self.dialog.detail_msg.stateText = "待打款";
          ...
          .axios({
            ...
            data: {
              ...
              params: {
                ...
                breakcontractReason: self.dialog.reject_msg.content
              }
            }
          })
          .then(response => {
          ...
          .axios({
            ...
            data: {
              ...
              params: {
                ...
                breakcontractReason: self.dialog.adopt_msg.content
              }
            }
          })
          .then(response => {
    ...
    async seeZip() {
      const self = this;
      if (self.dialog.detail_msg.id) {
        const { data: file } = await this.getImageZip(
          self.dialog.detail_msg.id
        );
