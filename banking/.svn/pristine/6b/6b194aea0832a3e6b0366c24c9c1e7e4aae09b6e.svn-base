<template>
  <el-main v-loading="loading">
    ...
    <div   class="tableDate">
      <el-table :data="listResult.list" stripe>
        ...
        <el-table-column
          ...
          width="140">
          <template slot-scope="scope">
            <el-button @click="confirmMoney(scope.row)" v-show="isButtonAuthor('75dca53f49394d0391b53e439ebe7d29')"
            >确认打款</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog
     ...
     <!-- <div class="upload_msg" ref="code" id="code">
        <qr-code :text="codeUpload" :size="120" color="#000" bg-color="#fff" class="codebody"  v-show="codeIsShow"></qr-code>
        <span @mouseenter="codeIsShow=true" @mouseleave="codeIsShow=false">二维码上传</span>
      </div>-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        //表格信息
        uploadUrl: this.$store.state.api.picture.uploadImg,
        picJson:[] //图片数组
      };
    },
    created() {
    ...
    methods: {
       ...
       */
      computedCodeUrl(){
        var basurl= this.$store.state.api.picture.picIpStr;
        var picData={
        ...
        this.codeUpload =`
           ${basurl}${picData.id}&typeId=${picData.type}&subTypeId=${picData.subType}
        `
      },
      /**
       *  获取数据
