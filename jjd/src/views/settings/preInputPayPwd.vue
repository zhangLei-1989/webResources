  ...
  export default {
    ...
    methods: {
      ...
      doSomething () {
        ...
        this.cur = num
//                this.$el.querySelector(`div.keybord-btn.b${num}`).hover = true
        if (num === 'd') {
      ...
      confirm () {
        let path = ''
        if (this.type === 'modify') {
          path = `/modifyPayPwd/${md5(md5(this.realPwd))}`
        } else if (this.type === 'add') {
          path = `/addPayPwd/${md5(md5(this.realPwd))}`
        }
        this.$router.go({
          path,
          replace: true
...
<style lang="scss"
  ...
  .bc {
    background: #D1D5DB url("../../assets/ac.svg") center no-repeat {
      size: 30px;
      position: center;
    }
  }
