<template>
  <div>
    ...
    <section v-if="guarantee">
      <div class="user">丁方（担保人）：{{guarantee}}</div>
    ...
    <section>
      <h3>十一、债权转让</h3>
      <div>
        出借人有权将本协议项下的部分或全部债权转让给人人信或借到平台上的其他注册用户，出借人委托人人信将债权转让事项通过向借款人在今借到平台注册手机号码发送短信或平台信息推送的方式通知借款人。短信或平台信息推送一经发出即视为已通知借款人。
      </div>
      <div>
    ...
    <section>
      <h3>十二、催收支持</h3>
      <div>
        借款人逾期30日未偿还借款的，为方便人人信为出借人提供诉讼服务，出借人可将本协议项下债权零对价转让给人人信，由人人信对借款人提起民事诉讼。上述诉讼所需费用（包括但不限于案件受理费、公告费、公证费等）在出借人与人人信签署债权转让协议时由人人信向债权人一次性收取。出借人拒绝缴纳上述诉讼费用或缴纳不足视为出借人自动放弃了相应的权利，人人信将停止后续诉讼服务，造成的后果由出借人自行承担。
      </div>
      <div>
    ...
    <section>
      ...
      <div>
        若甲方与丙方、乙方与丙方<span v-if="guarantee">、丁方与丙方</span>或甲乙丙<span v-if="guarantee">丁四</span><span v-else>三</span>方发生任何纠纷或争议，首先应友好协商解决，协商不成的，同意将纠纷或争议提交丙方住所地有管辖权的人民法院，或提交至丙方指定的仲裁委员会，相关的诉讼费、仲裁费、律师代理费等因追索权利而发生的合理费用均由败诉方承担。
      </div>
      <div>
        若甲方与乙方<span v-if="guarantee">、甲方与丁方、乙方与丁方</span>发生任何纠纷或争议，首先应友好协商解决，协商不成的，同意将纠纷或争议提交提起诉讼/仲裁一方住所地有管辖权的人民法院或仲裁委员会，相关的诉讼费、仲裁费、律师代理费等因追索权利而发生的合理费用均由败诉方承担。
      </div>
      <div>
        本协议中所使用的定义，除非另有规定，否则应适用今借到平台释义规则，今借到平台对本文定义享有最终解释权。
      </div>
    </section>
    <div :class="{ 'seal': borrower}"><br>签订日期：{{crtDate}}<br><br><br></div>
  </div>
</template>
...
<style rel="stylesheet/scss"
  ...
  .seal {
    background: url("../../../static/seal.svg") no-repeat;
